        ...
        var html = template.html(title, list,
          `
              ...
              <form action="/author/update_process" method="post">
                  ...
                  <p>
                      <input type="text" name="name" value="${sanitize(author[0].name)}"  placeholder="name">
                  </p>
                  <p>
                      <textarea name="profile"  placeholder="description">${sanitize(author[0].profile)}</textarea>
                  </p>
                  <p>
...
exports.delete_process = function(request, response) {
  ...
  request.on('end', function() {
    var post = qs.parse(body);
    connection.query(`DELETE FROM topic WHERE author_id=?`, [post.id],
